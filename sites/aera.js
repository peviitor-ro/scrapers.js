...
const getJobs = async () => {
  ...
  await Promise.all(
    items.map(async (item) => {
      ...
      const { city: c, county: co } = await _counties.getCounties(translate_city(city.toLowerCase()));
      if (c) {
        cities.push(c);
...
const run = async () => {
  const company = "Aera";
  const logo = "https://lever-client-logos.s3.amazonaws.com/dfa07fbc-23b8-4677-9df5-6bb3d39f07db-1511999864878.png";
